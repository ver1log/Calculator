...
void Calculator::run()
{
    ...
    while (running)
    {
        ...
        while (SDL_PollEvent(&event))
        {
            ...
            if (barDisplay.length() < 9)
            { // if the bar is not full allow any key
                if (event.type == SDL_KEYDOWN)
                {
                    ...
                    case SDLK_BACKSPACE:
                        ...
                        if (barDisplay.length() == 1)
                        { // if there is a single character left, delete it and add a white space
                            barDisplay = " ";
                        }
                        else
                        {
            ...
            else if (barDisplay.length() >= 9)
            { // if the bar is full only allow the delete key,
                if (event.type == SDL_KEYDOWN)
                {
                    switch (event.key.keysym.sym)
                    {
        ...
        SDL_SetRenderDrawColor(renderer, 100, 7, 130, 0); // background color
        ...
        { // stop one early, before the delete button
...
void Calculator::pressButton(int MouseX, int MouseY, string &barMessage)
{
    ...
    for (int i = 0; i < rects.size(); i++)
    {
        if (SDL_PointInRect(&MousePos, &rects[i]))
        {
            ...
            if (barMessage.length() < 9)
            { // allow any key
                if (iter->second == "DEL")
                {
                    if (barMessage.length() == 1)
                    { // if there is a single character left, delete it and add a white space
                        barMessage = " ";
                    }
                    else
            ...
            else if (barMessage.length() >= 9)
            { // only allow the delete and clear button
                if (iter->second == "DEL")
                {
                    barMessage.pop_back();
                }
...
bool Calculator::isValidInput(string s)
{
    ...
    if (s == " =")
    { // if the user only enters the equal sign
        return true;
    } // checks if the first character is an operator(except the ".")
    else if (s[1] == operations[0][0] || s[1] == operations[1][0] || s[1] == operations[2][0] || s[1] == operations[3][0])
    { // checks that the first element of the input is not an operation
        return false;
    ...
    { // checks that none of the operations come back to back
        // cout << ":" << s << "|\n";
        int first = 0;
        int next = first + 1;
        while (next < s.length())
        {
            for (int i = 0; i < 4; i++)
            { // checks if two consecutive characters both are also within the operations array
        ...
        int numberofDecimals = [&](string str)
        { for(char c:s){ if(c == '.'){count++;}} return count; }(s);
        count = 0;
...
void Calculator::parseValidInput(string &s)
{
    ...
    double res = 0;
    while (str.size() != 1)
    { // continue while we did not get the final result
        cout << "entered whole\n";
        int operatorIndex = 0;
        while (MD[0] != "0" && MD[1] != "0")
        { // do the multpily and divide operation until there are still
        ...
        if (MD[0] == "0" && MD[1] == "0")
        { // start doing the add and sub operation once the multiply and divide operator are not in the list
            while (AS[0] != "0" && AS[1] != "0")
            {
                for (int i = 0; i < str.size(); i++)
                {
...
vector<string> Calculator::convertToList(string s)
{
    ...
    while (lookupIndex < s.length())
    { // while we can still look at the elements
        if (s[lookupIndex] == '*' || s[lookupIndex] == '/' || s[lookupIndex] == '+' || s[lookupIndex] == '-')
        {                                                                           // if we encouter the special characters
        ...
        { // if we dont encounter an operand, only increment the looking ahead index, current stays in the same position
            lookupIndex++;
        }
    }
